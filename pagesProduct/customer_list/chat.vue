<template>
	<div class="broadcast-details" :style="'height:'+windowH+'px'">
		<view class="hd-wrapper" :class="active === true ? 'on' : ''" :style="viewColor" @click="stoppress">
			<view class="store-wrapper" v-if="storeInfo">
				...
				<view class="acea-row store-opeation">
					<navigator v-if="hide_mer_status != 1 && storeInfo.mer_id" :url="'/views/pages/home/store?mer_id='+storeInfo.mer_id" class="link" open-type="redirect">进店</navigator>
					<text v-if="storeInfo.mer_phone" class="iconfont icon-dadianhua01" @click="goCustomer"></text>
				</view>	
			</view>
			 ...
			 @scrolltoupper="bindScroll">
				<div class="chat" ref="chat">
						...
						<block v-for="(item,j) in items.children" :key="item.service_log_id">
							...
							<div class="item acea-row row-top" v-if="item.send_type == 0">	
								...
								<div class="text">								
									<div class="acea-row">
										<!--退款订单链接-->
										<navigator v-if="item.msn_type === 6 && item.refundOrder && item.refundOrder.refund_order_id" open-type="navigate" :url="'/pagesOrder/order-details/index?order_id='+item.refundOrder.order_id">
												...
												<div class="broadcast-details_order noPad" v-for="(val, inx) in item.refundOrder.refundProduct" :key="item.service_log_id">
													<div class="broadcast-details_box noPad" v-if="inx == 0">
														<div class="broadcast_details_img">
															<image :src="val.orderProduct.cart_info.product.image" />
															<div class="broadcast_details_model">
																{{item.refundOrder.refund_num}}件商品
															</div>
														</div>
														<div class="broadcast_details_picBox noPad">
															<div class="broadcast_details_tit">{{val.orderProduct.cart_info.product.store_name}}</div>
										...
										<div v-if="item.msn_type === 5 && item.orderInfo && item.orderInfo.order_id">
											...
											<navigator :url="'/packages/mall/views/pages/admin/orderDetail/index?id='+item.orderInfo.order_id+'&mer_id='+item.orderInfo.mer_id" open-type="open-type" class="conter acea-row row-middle">
										...
										<div class="conter acea-row row-middle" v-if="item.msn_type === 4 && item.product">
											<div class=" noPad">
												<navigator class="acea-row row-column-around noPad" v-if="item.product.product_id" :url="`/pagesProduct/goods_details/index?id=${item.product.product_id}`" open-type="navigate">
										...
										<div class="conter acea-row row-middle" v-if="item.msn_type === 7 && item.presell && item.presell.product">
											<div class=" noPad">
												<navigator class="acea-row row-column-around noPad" v-if="item.presell.product_presell_id" :url="`/packages/mall/views/pages/activity/presell_details/index?id=${item.presell.product_presell_id}`" open-type="navigate">
													<div class="broadcast_details_img_no">
										...
										<div class="conter acea-row row-middle" v-if="item.msn_type === 8 && item.productGroup && item.productGroup.product">
											<div class=" noPad">
												<navigator class="acea-row row-column-around noPad" v-if="item.productGroup.product_group_id" :url="`/pagestivity/combination_details/index?id=${item.productGroup.product_group_id}`" open-type="navigate">
													<div class="broadcast_details_img_no">
								...
								<div class="text textR">
									...
									<div class="acea-row ">
										...
										<navigator v-if="item.msn_type === 6 && item.refundOrder.refund_order_id" open-type="navigate">
											...
											<div class="conter acea-row row-middle">
												<div class="broadcast-details_order noPad" v-for="(val, inx) in item.refundOrder.refundOrderProduct" :key="val.id">
													<div class="broadcast-details_box noPad" v-if="inx == 0">
														<div class="broadcast_details_img">
															<image :src="val.orderProduct.cart_info.product.image" />
															<div class="broadcast_details_model">
										...
										<div v-if="item.msn_type === 5 && item.orderInfo.order_id">
											...
											<navigator :url="'/packages/mall/views/pages/admin/orderDetail/index?id='+item.orderInfo.order_id+'&mer_id='+item.orderInfo.mer_id" open-type="navigate" class="conter acea-row row-middle">
												<div class="broadcast-details_order noPad" v-for="(val, inx) in item.orderInfo.orderProduct" :key="val.id">
													<div class="broadcast-details_box noPad" v-if="inx == 0">
														<div class="broadcast_details_img">
															<image :src="val.cart_info.product.image" />
										...
										<div class="conter acea-row row-middle" v-if="item.msn_type === 4 && item.product">
											<div class=" acea-row row-column-around noPad">
												<navigator class="acea-row row-column-around noPad" v-if="item.product.product_id" :url="`/pagesProduct/goods_details/index?id=${item.product.product_id}`">
													<div class="broadcast_details_img_no">
														<image :src="item.product.image" />
													</div>
													<div class="broadcast_details_picBox_no noPad">
										...
										<div class="conter acea-row row-middle" v-if="item.msn_type === 8 && item.productGroup && item.productGroup.product">
											<div class=" acea-row row-column-around noPad">
												<navigator class="acea-row row-column-around noPad" v-if="item.productGroup.product_group_id" :url="`/packages/mall/views/pages/activity/combination_details/index?id=${item.productGroup.product_group_id}`" open-type="navigate">
													<div class="broadcast_details_img_no">
														<image :src="item.productGroup.product.image" />
													</div>
													<div class="broadcast_details_picBox_no noPad">
						...
						<block v-for="(item,j) in items.children" :key="item.service_log_id">
							...
							<div class="item acea-row row-top" v-if="item.send_type == 1">
								...
								<div class="text">
									...
									<div class="acea-row">
										<!--退款订单链接-->
										<navigator v-if="item.msn_type === 6 && item.refundOrder.refund_order_id"  open-type="navigate" :url="'/pagesOrder/order-details/index?order_id='+item.refundOrder.order_id">
											<div class="broadcast-details_num">
												<text>退款单号：{{ item.refundOrder.refund_order_sn }}</text>
											</div>
											<div class="conter acea-row row-middle">
												<!-- :key="item.service_log_id" -->
												<div class="broadcast-details_order noPad" v-for="(val, inx) in item.refundOrder.refundOrderProduct">
													<div class="broadcast-details_box noPad" v-if="inx == 0">
														<div class="broadcast_details_img">
															<image :src="val.orderProduct.cart_info.product.image" />
															<div class="broadcast_details_model">
														...
														<div class="broadcast_details_picBox noPad">
															<div class="broadcast_details_tit">{{val.orderProduct.cart_info.product.store_name}}</div>
															<div class="broadcast_details_pic">
																￥{{ val.orderProduct.cart_info.productAttr.price }}
															</div>
														</div>
													</div>
												</div>
										...
										<div v-if="item.msn_type === 5 && item.orderInfo.order_id">
											...
											<navigator :url="'/pagesOrder/order-details/index?order_id='+item.orderInfo.order_id" open-type="navigate" class="conter acea-row row-middle">
												<!-- :key="item.service_log_id" -->
							...
							<div class="item acea-row row-top row-right" v-else>
								<div class="text textR">	
									...
									<div class="acea-row" @longpress="longTime(items,index,item,j)">
										...
										<navigator v-if="item.msn_type === 6 && item.refundOrder.refund_order_id" open-type="navigate" :url="'/pagesOrder/order-details/index?order_id='+item.refundOrder.order_id">
											<div class="broadcast-details_num">
												<text>退款单号：{{ item.refundOrder.refund_order_sn + 'abc'}}</text>
											</div>
											<div class="conter acea-row row-middle">
												<!-- :key="item.service_log_id" -->
					...
					<div class="broadcast-details_order footer_count">
						...
						<div class="broadcast_box" v-if="orderId && orderInfo.order_id">
							...
							<div class="broadcast-details_box">
								<div class="broadcast_details_img">
									...
									<image :src="(cartInfo.cart_info.productAttr && cartInfo.cart_info.productAttr.image) || cartInfo.cart_info.product.image" />
								</div>
								<div class="broadcast_details_picBox">
						...
						<!-- 退款订单 -->
						<div class="broadcast_box" v-if="refund_order_id && refundDetail_.refund_order_id">
							<text class="iconfont icon-guanbi" @click.stop="refund_order_id = ''"></text>
							<div class="broadcast-details_num broadcast_num">
								<text>退款单号：{{ refundDetail_.refund_order_sn }}</text>
							</div>
							<div class="broadcast-details_box">
								<div class="broadcast_details_img">
									<image :src="refundDetail_.refundOrderProduct[0].orderProduct.cart_info.product.image" />
								</div>
								<div class="broadcast_details_picBox">
									<div class="broadcast_details_tit">
										{{ refundDetail_.refundOrderProduct[0].orderProduct.cart_info.product.store_name }}
									</div>
									<div>
										<div class="broadcast_details_pic">
											共{{ refundDetail_.refund_num }}件商品，
											合计 <text class="p-color">￥{{ refundDetail_.refund_price}}</text>
										</div>
		...
		<div class="footerCon" :class="active === true ? 'on' : ''" ref="footerCon" :style="{ bottom: 0 - safeAreaInsetsBottom + 'px' }">
			<form>
				<div class="footer acea-row row-between row-bottom" ref="footer">
					...
					<view class="input_count">
						<input type="text" placeholder-class='placeholder' placeholder="输入内容" class="input" ref="inputRef" v-show="!voice" @input="bindInput" @keyup="keyup"
						 @focus="focus" cursor-spacing="20" @confirm="sendTest" v-model="textCon">
		...
		<!-- <div class="recording" v-if="recording">
			<image src="@/packages/mall/static/images/recording.png" />
		</div> -->
	</div>
</template>
<script setup>
	...
	import emojiList from "@/infrastructure/utils/emoji.js";
	import { useChat } from "@/hook/chat.js";
	import { getMerHistory } from "@/business/api/modules/user-old.ts";
	import { getChatRecord, hasServiceApi } from "@/business/api/modules/user.ts";
	// import { getProductDetail, getPresellProductDetail, getStoreDetail } from "@/business/api/modules/store-old.ts";	
	import { getPresellProductDetail } from "@/business/api/modules/store-old.ts";	
	import { getProductDetail } from "@/business/api/modules/store.ts";	
	import { getCombinationDetail } from "@/business/api/modules/activity.ts";
	import { getOrderDetailApi, refundDetailApi } from "@/business/api/modules/order.ts";
	...
	const safeAreaInsetsBottom = uni.getSystemInfoSync().safeAreaInsets.bottom;
	...
	const url = ref(`${HTTP_REQUEST_URL}/api/upload/image`)

	const headers = ref({
		"Authori-zation": "Bearer " + Store.token
	})
	const emojiGroup = ref(chunk(emojiList, 21))
	const active = ref(false)
	const voice = ref(false)
	const speak = ref("按住 说话")
	...
	const percent = ref(0)
	const footerConH = ref(0)
	const footerH = ref(1.08)
	...
	//图片大小
	const upload_max = ref(2) 
	//上传的图片地址
	const uploadImages = ref([])
	//展示的图片地址
	const uploads = ref([])
	// 超出限制数组
	const exceeded_list = ref([])
	const windowH = ref(0)
	const isBQ = ref(false)
	const scrollTop = ref(0) //滚动距离
	const textCon = ref('') //文字
	const iconColor = ref('#CCCCCC')
	const mer_id = ref(0) //商铺id
	const refund_order_id = ref(0) //退款订单id
	const refundDetail_ = ref('') //退款订单详情
	const userId = ref(0) //客服----用户id
	const pageType = ref(0) //页面区分
	const status = ref(false) //砍价列表是否获取完成 false 未完成 true 完成
	const storeInfo = ref('')
	...
	watch(() => textCon.value, (nval, oVal) => {
		if (nval == "") {
			iconColor.value = '#CCCCCC'
		} else {
			iconColor.value = '#86C956'
		}
	})
	...
	onUnload(()=>{
		...
		uni.$off(["socket_close"])
		back_timer.value && clearTimeout(back_timer.value)
	})

	onLoad((option)=>{
		toUid.value = option.uid || 0;
		productId.value = parseInt(option.productId) || 0;
		presellId.value = parseInt(option.presellId) || 0;
		combinationId.value = parseInt(option.combinationId) || 0;
		orderId.value = option.order_id || ""
		mer_id.value = option.mer_id || 0
		refund_order_id.value = option.refund_order_id || 0
		userId.value = option.userId || 0
		uni.getSystemInfo({
	...
	function commonSetData(data) {
		...
		if(history.value.length>0 && history.value[history.value.length-1]['children'].length > 0){
			console.log("触发----》", data)
			let index = history.value[history.value.length-1]['children'].length-1
			let item = history.value[history.value.length-1]['children'][index]
		
			if(data.send_time - item.send_time > 300){
				history.value.push({time: data.create_time,children:[data]})
			}else{
				history.value[history.value.length-1]['children'].push(data);
			}
		}else{
			history.value.push({time: data.create_time,children:[data]})
		}	
	...
	onMounted(()=>{
		httpUrl.value = `${HTTP_REQUEST_URL}/static/look.png`;
		if (isLogin.value) {
			if (userId.value == 0) {
				// 用户
				getHistory();
	...
	function userDetail(uid){
		wsEnd()
		uni.redirectTo({
			url: `/packages/mall/views/pages/chat/customer_info/index?uid=${uid}&mer_id=${mer_id.value}`
		})
	}

	/*判断店铺是否有在线客服*/
	function hasService(){
		hasServiceApi({ mer_id: mer_id.value }).then(res => {
			height();
			if (userId.value == 0) {
				// 用户
				getHistory();
	...
	function goCustomer(){
			uni.makePhoneCall({	
			// 手机号
			phoneNumber: storeInfo.value.mer_phone, 
			// 成功回调
			success: (res) => {},
			// 失败回调
			fail: (res) => {}		
	...
	// 获取店铺详情
	async function getStoreDetail_() {
		try {
		...
		// getStoreDetail(mer_id.value).then(res => {
	...
	function wsEnd() {
		...
		isBack.value = true
		// socket.value && socket.value.onClose();
		SocketTask.value && SocketTask.value.close();
	}

	// 删除数组中已撤回的消息
	function deleteMsg(data){
		history.value.forEach(function(item, index) {
	...
	function uploadImg() {
		util.uploadImageOne('post/uploadImage', function(res) {
			sendMsg(res.data.image_url, 3)
		});
	}

	/*长按事件*/
	function longTime(items,index,item,j_){
		item.longpress = true
		history.value[index.value]['children'][j.value]['longpress'] = false
		press.value = true
		index.value = index;
		j.value = j_
		history.value[index.value]['children'][j.value]['longpress'] = true
	}
	...
	function reverstMsg(item) {
		sendMsg(item.service_log_id,100)
		setTimeout(function(){
	...
	function getOrderInfo() {
		...
		getOrderDetailApi({ order_id: orderId.value }).then(res => {
			orderInfo.value = res.data;
			if (orderInfo.value.orderProduct.length) {
				cartInfo.value = orderInfo.value.orderProduct[0];
			}
		});
	}

	function getproductInfo() {
		if (!productId.value && !presellId.value && !combinationId.value) return;
		if(presellId.value || combinationId.value){
			if(presellId.value){
				getPresellProductDetail(presellId.value).then(res => {
					productInfo.value = res.data;
				});		
			}else if(combinationId.value){
				getCombinationDetail(combinationId.value).then(res => {
	...
	function scroll() {
		if (window.scrollY < 300 && !loaded.value && !loading.value)
		getHistory();
	}
	...
	function getHistory() {
		...
		getChatRecord({
				...
				let arr = getChatTime(data.items)
				let newArr = []
				for(var j in arr){
					let key = j
					let item  = {}
				...
				history.value = newArr.concat(history.value);
				...
				page.value++;
				loading.value = false;
				loaded.value = data.length < limit.value;
			})
			.catch(err => {
				util.Tips({
	...
	// 客服聊天记录
	function getMerHistory_() {
		if (loading.value || loaded.value) return;
		loading.value = true;
		const last_id = history.value.length ? history.value[0].service_log_id : '';
		getMerHistory(userId.value, mer_id.value, {
				page: page.value,
				page_size: limit.value,
				last_id
			})
	...
	function keyup() {
		...
		if (textCon.value.length > 0) {
			sendColor.value = true;
		} else {
			sendColor.value = false;
		}
	}
	...
	function replace_em(str) {
		str = str.replace(/\[em-([\s\S]*)\]/g, "<span class='em em-$1' style='background-image:url(" + httpUrl.value + ")'></span>");
		return str;
	}
	...
	function sendProduct() {
		if(presellId.value || combinationId.value){
			if(presellId.value){
				sendMsg(presellId.value, 7);
				presellId.value = 0;									
			}else if(combinationId.value){
				sendMsg(combinationId.value, 8);
				combinationId.value = 0;
			}
		}else{
			sendMsg(productId.value, 4);
			productId.value = 0;	
		}
		productInfo.value = {};
	}
	...
	function sendOrder() {
		sendMsg(orderId.value, 5);
		orderId.value = 0;
		orderInfo.value = {};
	}

	function sendRefundOrder() {
		sendMsg(refund_order_id.value, 6);
		refund_order_id.value = 0;
		refundDetail_.value = {};
	}
	...
	function end() {
		clearTimeout(timeOutEvent.value);
		if (timeOutEvent.value !== 0 && longClick.value === 0) {
			//点击
			//此处为点击事件----在此处添加跳转详情页
		}
		speak.value = "按住 说话";
	...
	function height() {
		...
		setTimeout(res => {
			(!isBack.value) && info.boundingClientRect(function(data) { //data - 各种参数
				scrollTop_ = data.height
				if (active.value) {
					scrollTop.value = parseInt(scrollTop_) + 500
				} else {
					scrollTop.value = parseInt(scrollTop_) + 100
				}
			}).exec()
		}, 1000)
	}
...
<style lang="scss" scoped>
	...
	// @import url("@/plugin/emoji-awesome/css/google.min.css");
		@import "@/static/emoji-awesome/css/tuoluojiang.css";
	.noPad {
		width: 100%;
		padding: 0 !important;
