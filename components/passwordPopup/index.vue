<template>
	<view>
		<view class='password-window' :class='window_==true?"on":""' :style="{ 'background-image': `url(${domain}/static/images/passwordPop.png)`}">
		...
		<!-- <view class='mask' catchtouchmove="true" :hidden="window_==false" style="z-index: 999;"></view> -->
		<view class='mask' catchtouchmove="true" v-if="window_" style="z-index: 999;"></view>
	</view>
</template>
...
<script setup>
	...
	import { pwdResolution } from '@/business/api/modules/user.ts'
	...
	const copyPwd = computed(() => {
		return Store.copyPwd
	})
	const uid = computed(() => {
		return Store.uid
	})

	watch(()=>copyPwd.value,(nVal,oVal)=>{
		if (nVal) resolution(nVal)
	},{
		immediate: true,
	})
	...
	function resolution(data){
		...
		pwdResolution(data).then(res => {
			if(res.data.user && res.data.user.uid != uid.value && com !=res.data.com){
				window_.value = true;
				pwdInfo.value = res.data;
				userInfo.value = res.data.user;
				storeInfo.value = res.data.data;
				if (res.data.activity_id ) storeInfo.value.activity_id = res.data.activity_id 
				if (res.data.product_type ) storeInfo.value.product_type = res.data.product_type 
				// this.$store.commit("PARSE_PWD", null)
				Store.PARSE_PWD(null)
