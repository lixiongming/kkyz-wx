<template>
	<view class="orderGoods" :style="Store.viewColor">
...
<script lang="js" setup>
	...
	import { initiateAssistApi } from '@/business/api/modules/activity';
	...
	function evaluateTap(unique,orderId){
		if(props.welfare_post_id > 0){
	       uni.redirectTo({
		    	url:`/pagesOrder/goods-comment-con/index?uni=${unique}&order_id=${orderId}&welfare_post_id=${props.welfare_post_id}`
		   })
		}else{
	        uni.redirectTo({
		    	url:`/pagesOrder/goods-comment-con/index?uni=${unique}&order_id=${orderId}`
		    })
		}
	...
	function isPayBalance(){
		if(props.orderData.status === 10){
			if(new Date() < new Date(props.orderData.presellOrder.final_start_time)){
				isTimePay.value = false; //未开始
			}else if((new Date() >= new Date(props.orderData.presellOrder.final_start_time)) && (new Date() <= new Date(props.orderData.presellOrder.final_start_time)) ){
				isTimePay.value = true; //立即支付
			}
		}
	}
	function jumpCon(item){
	...
	function refund(item){  
		...
		openOrderRefundSubscribe().then(() => {
			uni.hideLoading();
			if(props.evaluate == 0 || props.evaluate == 9 || props.orderData.is_virtual != 0){	
				uni.navigateTo({
					url:'/pagesOrder/refund/confirm?order_id='+props.orderId+'&type=1'+'&ids='+item.order_product_id+'&refund_type=1&order_type='+props.orderData.order_type
				})
			}else{
				uni.navigateTo({
					url:'/packages/mall/views/pages/users/refund/select?order_id='+props.orderId+'&type=1&order_type='+props.orderData.order_type+'&ids='+item.order_product_id
				})
			}
		}).catch(() => {
			uni.hideLoading();
