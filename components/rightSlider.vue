<template>
	<view class="right-wrapper" @touchmove.stop.prevent="moveStop" :style="localViewColor">
...
onMounted(() => {
 // 重要组件挂载后
 list.value = props.brandList
 showBox.value = props.status
 min.value = props.price_on
 max.value = props.price_off
 storeTypeList.value = [
 	{name: '全部', value: '',check: false},
 	{name: '自营', value: 'trader',check: false},
 	{name: '非自营', value: 'trader',check: false}
 ]
 storeTypeList.value[index.value]['check'] = true
 is_trader.value = storeTypeList.value[0]['check'] ? '' : storeTypeList.value[1]['check'] ? 1 : 0
});

function bindChenck(item){
	item.check = !item.check
	arrFilter()
}

function bindChenckType(item,indexs){	
	storeTypeList.value = [
		{name: '全部', value: '',check: false},
		{name: '自营', value: 'trader',check: false},
		{name: '非自营', value: 'trader',check: false},
	]
	storeTypeList.value[indexs]['check'] = true
	is_trader.value = storeTypeList.value[0]['check'] ? '' : this.storeTypeList[1]['check'] ? 1 : 0
	index.value = indexs
}

function arrFilter(){
	activeList.value = list.value.filter(item=>{
...
function confirm(){
 arrFilter()
	let obj = {
		brandList:activeList.value,
		price_on:min.value,
		price_off:max.value,
		status:false,
		is_trader: is_trader.value
	}
	showBox.value = false
	emit('confirm',obj,index.value)				
}
function close(){
	showBox.value = false
	emit('close')
...
function moveStop(){}
	...
	// 		}
	// 	},
	// 	computed: mapGetters(['localViewColor']),
	// 	data(){
	// 		return {
	// 			min: '',
	// 			max:'',
