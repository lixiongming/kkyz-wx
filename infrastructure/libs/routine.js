...
class Routine 
{
	...
	async getCode(){
		...
		console.log(backUrlCRshlcICwGdGY,'服务商')
		return new Promise((resolve,reject)=>{
			// if(Cache.has(STATE_R_KEY)){
			// 	console.log(STATE_R_KEY,'STATE_R_KEY')
			// 	return resolve(Cache.get(STATE_R_KEY));
			// }
			// let wx_code = uni.getStorageSync('wx_code');
			// if (wx_code) {
			// 	return resolve(wx_code);
			// }
			uni.login({
				...
				success(res) {
					...
					// if (res.code) uni.setStorageSync('wx_code', res.code);
