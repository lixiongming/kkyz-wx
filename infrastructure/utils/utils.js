...
import dev from '@/infrastructure/config/env/development.js';
import pro from '@/infrastructure/config/env/production.js';
import store from '@/business/store/modules/mall.ts';
import {
	get_middle_token
} from '@/business/api/modules/store.ts';
import { history } from "@/business/api/modules/public.ts";
import { getconfig } from "@/business/api/modules/public-new.ts";
// #ifdef APP-PLUS
import permision from "./permission.js"
// #endif

const env = process.env.NODE_ENV || 'production';
let http_request_url = env === 'production' ? pro['baseURL'] : dev['baseURL']
export default {
	/**
	 * 字符串截取
	 * @obj 传入的数据
...
/**
	 ...
	 * 
	 */
	userPosterCanvas: function(arr2, nickname, spread,sitename, index, w, h, successFn) {
		let that = this;
		...
		uni.getImageInfo({
			...
			success: function(res) {
				...
				ctx.fillText(sitename, w * markx, h * marky);
				ctx.fillText('邀请码：' + spread, w * spreadx, h * spready);				
				ctx.save();
	 ...
	 * 
	 */
	storePosterCanvas: function(arr2, nickname, spread,sitename, index, w, h, successFn) {
		let that = this;
		const ctx = uni.createCanvasContext('myCanvas' + index);
		ctx.clearRect(0, 0, 0, 0);
		/**
		...
		uni.getImageInfo({
			...
			success: function(res) {
				...
				gradient.addColorStop(0, '#389bf8'); // 蓝色在顶部（y=0）
				gradient.addColorStop(1, '#f3f2f2');   // 红色在底部（y=canvas.height）
				...
				if (sitename.length > 12) {
					ctx.fillText(sitename.substring(0, 12), w * markx, h * marky);
					let str_ = ''
					if (sitename.substring(12).length > 12) {
						str_ = sitename.substring(12,24) + '...';
					} else {
						str_ = sitename.substring(12,24)
					}
					ctx.fillText(str_, w * markx, h * marky + 16);
				} else {
					ctx.fillText(sitename, w * markx, h * marky);
				}
				// ctx.fillText('邀请码：' + spread, w * spreadx, h * spready);				
	...
	uploadImageOne: function(opt, successCallback, errorCallback) {
		...
		uni.chooseImage({
			...
			success: async (res)=> {
				...
				let imageList = [];
				let urlPath = http_request_url + 'api/' + uploadUrl
				if (count === 1) {
					console.log(res.tempFiles[0].size/1024 )
					if(res.tempFiles[0].size/1024>uploadMaxSize * 1024) return uni.showModal({
						content: `图片大小超出限制${uploadMaxSize}MB`
					});
...
/**
		...
		if (arr.length > 1) {
			...
			if (url.indexOf("http") != -1) {
				...
				if(uni.getStorageSync('LOGIN_STATUS_TOKEN')){
					get_middle_token().then(({data})=>{
						 path=url+'?middle_token='+data.token
						 window.open(path)
					})
				}else{
					     window.open(path)
 ...
 export const  shareEvent= (url = `${uni.$config.shareAppUrl}/views/pages/mine/pages/appDownload/index?parent_code=${ uni.getStorageSync("USER_CODE")}`)=>{
...
export const  shareH5= (url)=>{
	uni.setClipboardData({
		data: 'http://channels.mdd3.cn/#/views/pages/h5/index?path=' + encodeURIComponent(url), 
		success: function () {
		  console.log('success');
			  uni.showToast({
			   title:'分享链接已复制',
...
export const getconfigApi = () => {
	getconfig().then((res)=>{
		const Store = store();
		console.log(res)
		Store.globalData = res.data
		uni.setStorageSync('GLOBAL_DATA', res.data);
		Store.VIEW_COLOR(res.data.global_theme.theme)
		Store.KEY_COLOR("_" + res.data.global_theme.type)
	} )
}
