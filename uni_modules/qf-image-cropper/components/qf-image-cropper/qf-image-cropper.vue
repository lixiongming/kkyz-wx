<template>
	<view class="image-cropper" :style="{ zIndex }" @wheel="cropper.mousewheel">
		...
		<view class="fixed-bottom safe-area-inset-bottom" :style="{ zIndex: initData.area.zIndex + 99 }">
			...
			<view class="btns_box" v-else-if="!!imgSrc">
				<button class="btns_cls button" size="mini" @click="chooseImage">重新</button>
				<button class="btns_cls button" size="mini" @click="cancel">关闭</button>
				<button class="btns_cls button" size="mini" @click="cropClick">预览</button>
				<button class="btns_cls button" size="mini" @click="uploadImg">上传</button>
			</view>
			<block v-else>
				<view class="cancel" @click="cancel">取消</view>
				<!-- <button class="button" size="mini" @click="chooseImage">选择图片</button> -->
				<view class="choose-button" @click="chooseImage">选择图片</view>
			</block>
...
<script>
	...
	export default {
		...
		methods: {
			 ...
			 */
			canvasToTempFilePath(canvas, canvasId, successCallback) {
			...
			uploadImg() {
				...
				if(!this.use2d) {
					...
					ctx.clearRect(0, 0, this.canvansWidth, this.canvansHeight);
					this.draw2DImage(null, ctx, this.imgSrc, () => {
						this.canvasToTempFilePath(null, 'imgCanvas', (res) => {
							console.log("this.tempFilePath--->", res.tempFilePath);
							uni.$emit("upload-image", res.tempFilePath);
						});
				...
				query.select('#imgCanvas')
					...
					.exec((res) => {
						...
						this.draw2DImage(canvas, ctx, this.imgSrc, () => {
							...
							uni.$emit("upload-image", this.tempFilePath);
						});
					});
				// #endif
