    ...
    <view>
		<view class='order-details'>
			...
			<view class="order-wrapper" v-for="(item,index) in orderInfo.orderList" :key="index">
				<view class="title">{{item.merchant.mer_name}}<text class="iconfont icon-xiangyou"></text>
				</view>
					...
					<view v-for="(goods,j) in item.orderProduct" :key="goods.order_product_id">
						<view v-if="item.activity_type === 2">
							<view class="item">
								...
								<view class="num"><text class="font-color">￥{{goods.cart_info.productPresellAttr.presell_price}}</text><br></br>x{{goods.product_num}}</view>
							</view>
						...
						<view v-else class="item">
							...
							<view class="info-box">
								...
								<view class='price acea-row row-middle'>
									...
									<image v-if="goods.cart_info.productAttr.show_svip_price" class="svip-img" :src="imgUrlEvent('/images/wx-applet/mine/mine-bg.png','aliyunImgUrl')"></image>
								</view>
							</view>
			...
			<view class='wrapper'>
				<view class='item acea-row row-between'>
					<view>订单编号：</view>
					<view class='conter acea-row row-middle row-right'>{{orderInfo.group_order_sn}}</view>
				</view>
				<view class='item acea-row row-between'>
					<view>下单时间：</view>
				...
				<view class='item acea-row row-between'>
					<view>使用兑换积分：</view>
					<view class='conter'>{{orderInfo.super_integral}}</view>
				</view>
				<view class='item acea-row row-between'>
					<view>兑换积分抵扣：</view>
					<view class='conter'>￥{{orderInfo.super_integral_price}}</view>
				</view>
			</view>
			<view v-if="orderInfo.orderList && orderInfo.orderList[0].is_virtual == 1 && orderInfo.orderList[0].order_extend" class='wrapper'>
				<view v-for="(item,index) in orderInfo.orderList[0].order_extend" :key="index" v-show="item" class='item acea-row row-between'>
					<view>{{index}}：</view>
					<view v-if="!Array.isArray(item)" class='conter'>{{item}}</view>
					<view v-else class='conter virtual_image'>
						<image v-for="(pic,i) in item" :key="i" class="picture" :src="pic"></image>
					</view>
				</view>
			</view>
			<view class='wrapper'>
            ...
            <view class='footer acea-row row-right row-middle' v-if="showPay">
				<view class='bnt bgColor' @tap='goPay(orderInfo.group_order_id)'>立即付款</view>
			</view>
		</view>
	</view>
</template>
...
import { middle_login, groupOrderDetail, orderPay, bindUserRouteId } from '@/business/api/modules/mall.ts';
...
const showPay = computed(() => {
    return orderInfo.value.group_order_id == 0 || orderInfo.value.group_order_id
})
...
function getOrderInfo() {
    ...
    groupOrderDetail(order_id.value).then(res => {
...
async function login_mall (middle_token_,code) {
    const { data } = await middle_login({ middle_token: middle_token_ });
    uni.setStorageSync("mall_token",data.token);
    // 获取openid
    await bindUserRouteId({ code, appid: 'wx29df62cbac3abc9a' })
    // 获取订单详情
    getOrderInfo()
}
    ...
    try {
        orderPay(id_).then(res => {
            ...
            uni.requestPayment({
                ...
                success: (e) => {
                    uni.redirectTo({
                        url: `/pages/users/order_pay_status/index?order_id=${order_id.value}`
                    });
                },
                fail: (e) => {
                },
