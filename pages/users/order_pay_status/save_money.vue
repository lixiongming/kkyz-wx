<template>
      ...
      <view class="payment-status">
        ...
        <view class="wrapper">
          <view class="item acea-row row-between-wrapper">
            <view>订单编号</view>
            <view class="itemCom">{{ order_pay_info.order_no || '--' }}</view>
          </view>
          <view class="item acea-row row-between-wrapper">
            <view>订单说明：</view>
            <view class="itemCom">{{ order_pay_info.remark || '--' }}</view>
          </view>
          <view class="item acea-row row-between-wrapper">
            <view>下单时间：</view>
            <view class="itemCom">{{ order_pay_info.created_at || '--' }}</view>
          </view>
          <view class="item acea-row row-between-wrapper">
            <view>支付状态：</view>
            <view class="itemCom">{{ pay_status || '--' }}</view>
          </view>
          <view class="item acea-row row-between-wrapper">
            <view>价格：</view>
  ...
  const pay_status = computed(() => {
    if (order_pay_info.value.status == 0) {
        return '待支付'
    } else if (order_pay_info.value.status == 1) {
        return '已支付'
    } else if (order_pay_info.value.status == -1) {
        return '已取消'
    }
  ...
  function getOrderPayInfo() {
    mineApi.getSaveMoneyOrderDetail(orderId.value).then(res => {
        order_pay_info.value = res.data.order_info;
    }).catch(err => {});
  }
